...
from base_model import lowlightnet3 as EDnet
def L1_regularization(model,lamda=1e-4):
...
def cfg():
    ...
    parser.add_argument('--trainset', type=str, default='/root/autodl-tmp/LOLdataset/train/', help='location of trainset') 
    parser.add_argument('--testset', type=str, default='/root/autodl-tmp/LOLdataset/test/', help='location of testset') 
    
    parser.add_argument('--output', default='/root/autodl-tmp/output/', help='location to save output images')
    parser.add_argument('--modelname', default='continue_base_model_2', help='define model name')
...
def checkpoint(model, epoch, opt):
    save_folder = os.path.join('/root/autodl-tmp/model/models',opt.modelname)   
...
def main(opt):
    ...
    with open(txt_name, mode='w') as txt_write:
        ...
        #         Training             #
        # =============================#
        psnr_score_all, ssim_score_all,score_all=0,0,0
        for epoch in range(opt.start_iter, opt.nEpochs + 1):
            ...
            for iteration, batch in enumerate(training_data_loader, 1):
                ...
                L1_reg=0
                prediction = pred_t * 255.0
                prediction = prediction.clip(0, 255)
                NL_real=NL_t*255.0
                NL_real=NL_real.clip(0, 255)
                l2_loss=L2_loss(prediction,NL_real)*0.001
                loss = 1*ssim_loss +1*p_loss + 1*smoothloss+l2_loss
                ...
                writer.add_scalar('l2_loss', l2_loss, over_Iter)
            ...
            if psnr_score>psnr_score_all:
                psnr_score_all=psnr_score
                torch.save(lighten.state_dict(), '/root/new_dachuang/SMNet-main/model/'+opt.modelname+'_'+'psnr.pth')
            if ssim_score>ssim_score_all:
                ssim_score_all=ssim_score
                torch.save(lighten.state_dict(), '/root/new_dachuang/SMNet-main/model/'+opt.modelname+'_'+'ssim.pth')
            if psnr_score+ssim_score>score_all:
                score_all=psnr_score+ssim_score
