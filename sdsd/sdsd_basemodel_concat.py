...
class low_block(nn.Module):
    def __init__(self, dim=64,device='cuda:0'):
        ...
        self.outlayer4 = ConvBlock(4*dim,4*dim, 1, 1, 0, bias=True)
        self.transformer_max4=Transformer(dim=4*dim,depth=1,heads=8)
        self.transformer_avg4=Transformer(dim=4*dim,depth=1,heads=8)

        self.para4 = torch.nn.Parameter(torch.ones((1,4*dim,1,1)).to(device).requires_grad_()/2)
    def attention(self,feature):
        ...
        H_avg_atten=self.transformer_avg4(H_avg.permute(0,2,1)).permute(0,2,1) #[b,c,w]-[b,w,c]-[b,c,w]
        W_avg_atten=self.transformer_avg4(W_avg.permute(0,2,1)).permute(0,2,1) #[b,c,h]-[b,h,c]-[b,c,h]
        H_max_atten=self.transformer_max4(H_max.permute(0,2,1)).permute(0,2,1) #[b,c,w]-[b,w,c]-[b,c,w]
        W_max_atten=self.transformer_max4(W_max.permute(0,2,1)).permute(0,2,1) #[b,c,h]-[b,h,c]-[b,c,h]
    ...
    def forward(self,f_endecoder,f_endecoder2,mode='train',dim=64):
        # here is img1
        if mode=='train':
            fullres = f_endecoder
            ...
            # cat_all = torch.cat((ende_fullres_out,ende_halfres_up,ende_quarres_up_up),dim=1)
            # fusion_all = self.out_fushion(cat_all) 
            cat_all = torch.cat((ende_quarres_up_up,fullres,ende_quarres_up_up2,fullres2),dim=1)
            fusion_all=self.attention(cat_all)
            fusion_all=self.outlayer4(fusion_all)+cat_all
            
            fusion_all1,fusion_all2=fusion_all[:,:dim*2,:,:],fusion_all[:,dim*2:,:,:]
            
            
            fusion_out = self.out_conv2(fusion_all1)  
